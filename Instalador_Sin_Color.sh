...
select opt in "${options[@]}"; do 
    ...
    1 ) echo "$respuesta $opt" && Rspta_Nv9="Si"&& sleep 1.64 && clear;;
    2 ) echo "$respuesta $opt" && Rspta_Nv9="No"&& sleep 1.64 && clear;;
...
if [ $? -ne 0 ]; then 
	...
	if [ $? -ne 0 ]; then 
        rm -R ~/.guekho64/minecraft/.secret 
        hcentro "${rojo}${negritas}Algo salió mal, cerrando el programa...${null}";
        sleep 3.28
        exit
    else
...
select opt in "${options[@]}"; do 
    ...
    1 ) echo "$respuesta $opt" && Rspta_Nv8="Si"&& sleep 1.64 && clear;;
    2 ) echo "$respuesta $opt" && Rspta_Nv8="No"&& sleep 1.64 && clear;;
  # Add another option at the end, like "Quit" // $(( ${#options[@]}+1 )) ) echo "Goodbye!"; break;;
    *) echo "${negritas}${rojo}
Opción Inválida${null}";continue;;
...
( chmod +x $Secret/minecraft-offline ) 
if [ $? -ne 0 ]; then 
rm -R ~/.guekho64/minecraft/.secret 
	hcentro "${rojo}${negritas}Algo salió mal, cerrando el programa...${null}";
	sleep 3.28
...
( sudo mv $Secret/minecraft-offline -f /usr/bin/minecraft-offline ) 
if [ $? -ne 0 ]; then 
rm -R ~/.guekho64/minecraft/.secret 
	hcentro "${rojo}${negritas}Algo salió mal, cerrando el programa...${null}";
	sleep 3.28
...
( sudo mkdir /.guekho64/icons ) 
( sudo chmod -R 777 /.guekho64/icons) 
if [ $? -ne 0 ]; then 
rm -R ~/.guekho64/minecraft/.secret 
	hcentro "${rojo}${negritas}Algo salió mal, cerrando el programa...${null}";
	sleep 3.28
...
rm "$MinecraftHomeIcons/*" ; (wget --no-check-certificate --directory-prefix=$MinecraftHomeIcons http://www.rw-designer.com/icon-image/5547-64x64x32.png -O $MinecraftHomeIcons/ml.png)  ; sudo ln --symbolic /.guekho64/icons/ml.* /usr/share/icons/minecraft.png
...
( chmod +x $MinecraftHomeDesktop/minecraft-offline.desktop ) 
if [ $? -ne 0 ]; then 
rm -R ~/.guekho64/minecraft/.secret 
	hcentro "${rojo}${negritas}Algo salió mal, cerrando el programa...${null}";
	sleep 3.28
...
clear ; hcentro "${verde}${negritas}¡El Instalador Ha Finalizado con Éxito!${null}" ; sleep 1.64
fi;
...
select opt in "${options[@]}"; do 
    ...
    1 ) echo "$respuesta $opt" && Rspta_Nv8Premium="Si"&& sleep 1.64 && clear;;
    2 ) echo "$respuesta $opt" && Rspta_Nv8Premium="No"&& sleep 1.64 && clear;;
  # Add another option at the end, like "Quit" // $(( ${#options[@]}+1 )) ) echo "Goodbye!"; break;;
    *) echo "${negritas}${rojo}
Opción Inválida${null}";continue;;
...
( sudo mv $Secret/minecraft -f /usr/bin/minecraft ) 
if [ $? -ne 0 ]; then 
rm -R ~/.guekho64/minecraft/.secret 
	hcentro "${rojo}${negritas}Algo salió mal, cerrando el programa...${null}";
	sleep 3.28
...
( chmod +x $MinecraftHomeDesktop/minecraft.desktop ) 
if [ $? -ne 0 ]; then 
rm -R ~/.guekho64/minecraft/.secret 
	hcentro "${rojo}${negritas}Algo salió mal, cerrando el programa...${null}";
	sleep 3.28
...
select opt in "${options[@]}"; do 
    ...
    1 ) echo "$respuesta $opt" && Rspta_Nv8Both="Si"&& sleep 1.64 && clear;;
    2 ) echo "$respuesta $opt" && Rspta_Nv8Both="No"&& sleep 1.64 && clear;;
  # Add another option at the end, like "Quit" // $(( ${#options[@]}+1 )) ) echo "Goodbye!"; break;;
    *) echo "${negritas}${rojo}
Opción Inválida${null}";continue;;
...
clear ; hcentro "${verde}${negritas}¡El Instalador Ha Instalado con Éxito el Launcher Premium!${null}" ; sleep 3.28 ; clear
fi;
clear ; hcentro "${verde}${negritas}¡El Instalador Ha Instalado con Éxito el Launcher Offline!${null}" ; sleep 3.28 ; clear
fi;
...
Basium="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )" 
...
mkdir -p ~/.guekho64/minecraft/launchers/original 
mkdir -p ~/.guekho64/minecraft/launchers/offline 
mkdir -p ~/.guekho64/minecraft/icons 
mkdir -p ~/.guekho64/minecraft/desktop_files 
mkdir -p ~/.guekho64/minecraft/.secret 
...
while menu && $Rd_Fctn && Ko=$num && clear && [[ "$num" ]]  ; do
    [[ "$num" != *[![:digit:]]* ]] &&
    (( num > 0 && num <= ${#options[@]} )) ||
    { msg="${negritas}${rojo}
...
clear
		mkdir ~/.guekho64/minecraft/.secret 
		touch ~/.guekho64/minecraft/.secret/SOYOFICIAL 
		(wget --directory-prefix=$MinecraftHomeOriginal https://s3.amazonaws.com/Minecraft.Download/launcher/Minecraft.jar -O $MinecraftHomeOriginal/Minecraft.jar) 
		if [ $? -ne 0 ]; then 
rm -R ~/.guekho64/minecraft/.secret 
	hcentro "${rojo}${negritas}Algo salió mal, cerrando el programa...${null}";
	sleep 3.28
...
clear
		...
		read input
		(wget --no-check-certificate --directory-prefix=$MinecraftHomeOffline $input -O $MinecraftHomeOffline/Minecraft.jar) 
		if [ $? -ne 0 ]; then 
rm -R ~/.guekho64/minecraft/.secret 
	hcentro "${rojo}${negritas}Algo salió mal, cerrando el programa...${null}";
	sleep 3.28
...
clear
		mkdir ~/.guekho64/minecraft/.secret 
		touch ~/.guekho64/minecraft/.secret/SOYAMBOS 
		(wget --directory-prefix=$MinecraftHomeOriginal https://s3.amazonaws.com/Minecraft.Download/launcher/Minecraft.jar -O $MinecraftHomeOriginal/Minecraft.jar) 
		if [ $? -ne 0 ]; then 
rm -R ~/.guekho64/minecraft/.secret 
	hcentro "${rojo}${negritas}Algo salió mal, cerrando el programa...${null}";
...
select opt in "${options[@]}"; do 
    ...
    1 ) echo "$respuesta $opt" && Rspta_Nv6="Si"&& sleep 1.64 && clear;;
    2 ) echo "$respuesta $opt" && Rspta_Nv6="No"&& sleep 1.64 && clear;;
...
echo $Estado_PPA_no1wantdthisname_openjdk 
        ...
        if [ $Estado_PPA_no1wantdthisname_openjdk = $Inexistente ]   ; then
        ( (sudo apt-add-repository ppa:no1wantdthisname/openjdk-fontfix -y) ) 
        else
        Anti_PPA2
        if [ $Estado_PPA_no1wantdthisname_openjdk = $Inactivo ]   ; then
        ...
        elif [ $Estado_PPA_no1wantdthisname_openjdk = $Activo ]   ; then
        echo "" 
        fi
        fi
...
echo $Estado_PPA_no1wantdthisname 
        ...
        if [ $Estado_PPA_no1wantdthisname = $Inexistente ]   ; then
        ( (sudo apt-add-repository ppa:no1wantdthisname/ppa -y) ) 
        else
        Anti_PPA_a2
        if [ $Estado_PPA_no1wantdthisname = $Inactivo ]   ; then
        ...
        elif [ $Estado_PPA_no1wantdthisname = $Activo ]   ; then
        echo "" 
        fi
        fi
        
        sudo apt-get update 
        sudo apt-get install fontconfig-infinality openjdk-8-jre --force-yes -y 
		if [ $? -ne 0 ]; then 
rm -R ~/.guekho64/minecraft/.secret 
	hcentro "${rojo}${negritas}Algo salió mal, cerrando el programa...${null}";
	sleep 3.28
...
sleep 3.28
        ...
        read ;
        clear ; hcentro "${blanco}${negritas}La desventaja de esto, es que ${negritas}${rojo}No${null} ${blanco}${negritas}se podrán arreglar algunos errores${null}"
...
select opt in "${options[@]}"; do 
    ...
    1 ) echo "$respuesta $opt" && Rspta_Nv7="Si"&& sleep 1.64 && clear;;
    2 ) echo "$respuesta $opt" && Rspta_Nv7="No"&& sleep 1.64 && clear;;
    3 ) echo "$respuesta $opt" && Rspta_Nv7="Medio"&& sleep 1.64 && clear;;
  # Add another option at the end, like "Quit" // $(( ${#options[@]}+1 )) ) echo "Goodbye!"; break;;
    *) echo "${negritas}${rojo}
Opción Inválida${null}";continue;;
    ...
    break
        ...
        sudo apt-get clean 
        sudo apt-get update 
        sudo apt-get install openjdk-7-jre --force-yes -y 
		if [ $? -ne 0 ]; then 
            rm -R ~/.guekho64/minecraft/.secret 
	        hcentro "${rojo}${negritas}Algo salió mal, cerrando el programa...${null}";
	        sleep 3.28
...
( touch $MinecraftHomeDesktop/minecraft.desktop ) 
...
( chmod +x ~/.guekho64/minecraft/desktop_files/minecraft.desktop ) 
...
( touch $MinecraftHomeDesktop/minecraft-offline.desktop ) 
...
( chmod +x ~/.guekho64/minecraft/desktop_files/minecraft-offline.desktop ) 
...
( cat ~/.guekho64/minecraft/.secret/SOYAMBOS ) 
if [ $? -ne 0 ]; then 
    ( cat ~/.guekho64/minecraft/.secret/SOYOFICIAL ) 
    if [ $? -ne 0 ]; then 
        ( cat ~/.guekho64/minecraft/.secret/SOYOFFLINE ) 
        if [ $? -ne 0 ]; then 
            rm -R ~/.guekho64/minecraft/.secret 
            hcentro "${rojo}${negritas}Algo salió mal, cerrando el programa...${null}";
            sleep 3.28
...
select opt in "${options[@]}"; do 
    ...
    1 ) echo "$respuesta $opt" && Rspta_Nv3="Instalar"&& sleep 1.64 && clear;;
    2 ) echo "$respuesta $opt" && Rspta_Nv3="Desinstalar"&& sleep 1.64 && clear;;
...
if  [ "$Rspta_Nv3" = "Instalar" ]  ; then
	...
	if  [ "$Rspta_Nv3" = "Desinstalar" ]  ; then
		...
		rm ~/.local/share/applications/minecraft-offline.desktop 
		echo "
${negritas}${cyan}Introduzca su contraseña para continuar:${null}"
		sudo rm /usr/share/applications/minecraft.desktop 
		sudo rm /usr/share/applications/minecraft-offline.desktop 
		sudo rm /usr/bin/minecraft 
		sudo rm /usr/bin/minecraft-offline 
		sudo rm $DesktopFolder/"Minecraft Offline.desktop" 
		sudo rm $DesktopFolder/Minecraft.desktop 
		sudo rm $DesktopFolder/"minecraft offline.desktop" 
		sudo rm $DesktopFolder/"Minecraft offline.desktop" 
		sudo rm $DesktopFolder/"minecraft Offline.desktop" 
		sudo rm $DesktopFolder/"Minecraft Offline.desktop" 
		...
		sudo rm $DesktopFolder/"Minecraft Offline.desktop" 
		sudo rm $DesktopFolder/Minecraft.desktop 
		sudo rm ~/"Minecraft Offline.desktop" 
		sudo rm ~/Minecraft.desktop 
		sudo rm ~/"minecraft offline.desktop" 
		sudo rm ~/"minecraft Offline.desktop" 
		sudo rm ~/"Minecraft offline.desktop" 
		sudo rm ~/minecraft.desktop 
		sudo rm -R /.guekho64/icons 
...
select opt in "${options[@]}"; do 
    ...
    1 ) echo "$respuesta $opt" && Rspta_Nv4="Si"&& sleep 1.64 && clear;;
    2 ) echo "$respuesta $opt" && Rspta_Nv4="No"&& sleep 1.64 && clear;;
...
select opt in "${options[@]}"; do 
    ...
    1 ) echo "$respuesta $opt" && Rspta_Nv5="Si"&& sleep 1.64 && clear;;
    2 ) echo "$respuesta $opt" && Rspta_Nv5="No"&& sleep 1.64 && clear;;
...
if  [ "$Rspta_Nv5" = "Si" ]  ; then
        ...
        sudo pkill synaptic 
        sudo apt-get clean 
        sudo rm /etc/apt/sources.list.d/no1wantdthisname* 
        ( sudo apt-get update && sudo pkill synaptic ) 
        sudo apt-get install ppa-purge --force-yes -y 
        sudo apt-get clean 
...
Casa="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )" 
...
( sudo chmod go-w ~/.Acepto.txt ) 
( sudo chown root:root $HOME/.Acepto.txt ) 
sudo pkill synaptic 
sudo apt-get clean 
sudo apt-get install libnotify-bin libnotify4 software-properties-gtk --force-yes -y 
if [ $? -ne 0 ]; then 
rm -R ~/.guekho64/minecraft/.secret 
	clear
...
select opt in "${options[@]}"; do 
    ...
    1 ) echo "$respuesta $opt" && Rspta_Nv2="Si"&& sleep 1.64 && clear;;
    2 ) echo "$respuesta $opt" && Rspta_Nv2="No"&& sleep 1.64 && clear;;
    3 ) echo "$respuesta $opt" && Rspta_Nv2="Nunca"&& sleep 1.64 && clear;;
...
if  [ "$Rspta_Nv2" = "Si" ]  ; then
    ...
    sudo apt-get clean 
		sudo apt-get update 
        sudo apt-get install xfce4-terminal --force-yes -y 
        if [ $? -ne 0 ]; then 
rm -R ~/.guekho64/minecraft/.secret 
	clear
	echo "${negritas}${rojo}Ha ocurrido un error en la instalación del programa. Saliendo...${null}";
...
select opt in "${options[@]}"; do 
    ...
    1 ) echo "
$respuesta $opt" && Rspta_Nv1="Si" && sleep 1.64 && clear;;
    2 ) echo "
$respuesta $opt" && Rspta_Nv1="No" && sleep 1.64 && clear;;
