...
if [ $? -ne 0 ]; then 
    cat "/usr/lib/jvm/java-7-openjdk-amd64/bin/java"
	if [ $? -ne 0 ]; then 
...
hcentro "${null}${negritas}Soy guekho64, el creador de este Script para Instalar Minecraft en Ubuntu Linux (O derivados)${null}"
...
echo ""
hcentro "${negritas}${rojo}Si instalas más de un launcher Offline, el viejo launcher será reemplazado por el nuevo${null}"
echo ""
hcentro "${verde}${negritas}Presione ENTER para continuar${null}" ;
read ;
reset ;

hcentro "${negritas}${azul}Probando la conexiòn a${null} ${negritas}${verde}Internet${azul}... ${null}  "
...
${negritas}${verde}Presione ENTER al terminar${null}
...
echo "${verde}${negritas}Se ha descargado el Launcher Oficial correctamente${null}"
sleep 3.2
		fi;
...
echo "${rojo}${negritas}No seleccionó ningún tipo de Launcher${null}"
sleep 3.2
reset
...
reset
hcentro "${negritas}${verde}Ahora procederé a instalar las dependencias necesarias para la instalación de Minecraft${null}"
hcentro "${negritas}${verde}además de alguno que otro parche para garantizar la correcta funcionalidad de las fuentes${null}"
sleep 3.2
echo ""
hcentro "${negritas}${azul}Estos repositorios de programas son ajenos a los repositorios oficiales de Ubuntu y a mi${null}"
hcentro "${negritas}${azul}por lo que ${rojo}no me hago responsable de cualquier daño a su computadora${null}"
hcentro "${negritas}${azul}Aunque esta advertencia es un poco exagerada, es siempre necesaria ponerla, aunque, por experiencia propia${null}"
hcentro "${negritas}${azul}les puedo asegurar que estas fuentes externas ${blanco}JAMÁS ${azul}me han dado problema alguno${null}"
echo ""
function Menu_Nv6 {
...
sleep 3.2
        ...
        read ;
        hcentro "${cyan}${negritas}La desventaja de esto, es que ${negritas}${rojo}No${null} ${cyan}${negritas}se podrán arreglar algunos errores${null}"
...
Opción Inválida${null}";continue;;
    ...
    break
        ...
        hcentro "${negritas}${cyan}Trabajando. Esto llevará tiempo${null}"
        hcentro "${negritas}${verde}Si tienes las notificaciones activadas,serás notificado cuando esto termine ${null}"
		( (sudo apt update) && (sudo apt install openjdk-7-jre -y) ) > /dev/null 2>&1
		if [ $? -ne 0 ]; then 
            rm -R ~/.guekho64/minecraft/.secret > /dev/null 2>&1
	        hcentro "${rojo}${negritas}Algo salió mal, cerrando el programa...${null}";
	        sleep 3.2
            ...
            echo "${verde}${negritas}¡Se ha instalado la versión oficial de Java correctamente!${null}" 
            sleep 3.2
        fi;
...
reset
hcentro "${negritas}${cyan}Es probable que se le solicite su contraseña. Si es así, introdúzcala${null}"
echo ""
...
reset
sleep 1.64
echo "${negritas}${verde}Cargando...${null}";
sleep 1.64
reset
...
echo "${verde}${negritas}Al parecer usted seleccionò ambos Modos de Launcher${null}"
sleep 3.2
Ambos
fi;
...
if  [ "$Rspta_Nv5" = "Si" ]  ; then
        ...
        ( sudo apt update && sudo pkill synaptic && sudo apt install ppa-purge ) > /dev/null 2>&1
