...
(wget --no-check-certificate --directory-prefix=$MinecraftHomeIcons http://www.rw-designer.com/icon-image/5547-64x64x32.png -O $MinecraftHomeIcons/ml.ico) 
...
reset
		sleep 2
echo "${blanco}Bye${null}"
rm ~/.guekho64/minecraft/.secret/SOYOFFLINE 
		exit	
		fi;
...
(wget --no-check-certificate --directory-prefix=$MinecraftHomeIcons http://www.rw-designer.com/icon-image/5547-256x256x8.png -O $MinecraftHomeIcons/ml.ico) 
...
rm ~/.guekho64/minecraft/.secret/SOYOFICIAL 
...
read input
	...
	if  [ "$input" = "$yup" ]  ; then
		...
		(wget --directory-prefix=$MinecraftHomeOriginal https://s3.amazonaws.com/Minecraft.Download/launcher/Minecraft.jar) 
		if [ $? -ne 0 ]; then 
rm -R ~/.guekho64/minecraft/.secret 
	echo "${rojo}${negritas}Algo salió mal, cerrando el programa...${null}";
	sleep 3.2
...
echo "${negritas}${cyan}Trabajando. Esto puede llevarse un tiempo, asì que sea paciente...${null}"
		( (sudo apt-add-repository ppa:no1wantdthisname/openjdk-fontfix -y) && (sudo apt-add-repository ppa:no1wantdthisname/ppa -y) && (sudo apt update) && (sudo apt install fontconfig-infinality openjdk-8-jre -y) ) 
		if [ $? -ne 0 ]; then 
rm -R ~/.guekho64/minecraft/.secret 
	echo "${rojo}${negritas}Algo salió mal, cerrando el programa...${null}";
	sleep 3.2
...
${negritas}${cyan}Introduzca su contraseña para continuar:${null}"
		...
		sudo rm ~/Escritorio/Minecraft-Offline.desktop 
		sudo rm ~/Escritorio/Minecraft.desktop 
		sudo rm ~/Escritorio/minecraft-offline.desktop 
		sudo rm ~/Escritorio/Minecraft-offline.desktop 
		sudo rm ~/Escritorio/minecraft-Offline.desktop 
		sudo rm ~/Escritorio/Minecraft-Offline.desktop 
		sudo rm ~/Escritorio/minecraft.desktop 
		sudo rm ~/Desktop/Minecraft-Offline.desktop 
		sudo rm ~/Desktop/Minecraft.desktop 
...
(sudo apt-add-repository ppa:xubuntu-dev/xfce-4.12 -y && sudo apt update && sudo apt install xfce4-terminal -y ) 
if [ $? -ne 0 ]; then 
rm -R ~/.guekho64/minecraft/.secret 
	reset
